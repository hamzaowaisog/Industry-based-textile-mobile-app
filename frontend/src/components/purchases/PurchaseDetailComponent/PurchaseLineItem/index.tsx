...
import type { PurchaseLineItemProps } from '../../../../types/purchases.types';
import { styles } from './styles';

export const PurchaseLineItem = ({ line, index, isLast }: PurchaseLineItemProps) => {
  ...
  return (
    <View style={[styles.row, !isLast && styles.rowBorder]}>
      ...
      <View style={styles.info}>
        ...
        <Text style={styles.sub}>{`${line.qty ?? 0} × ${formatPKR(line.unitCost ?? 0)}`}</Text>
      </View>
      <AppAmount value={lineTotal} size={14} tone="debit" />
    </View>
  );
};
